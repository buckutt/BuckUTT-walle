...
products.products = function(app, dbConnection){
	app.get("/api/products/seller_id=:seller_id&buyer_id=:buyer_id&point_id=:point_id", function(req, res){
		async.waterfall([
            function(callback){
            ...
            function(periods, callback){
                var query = "SELECT obj.obj_id, obj.obj_name, obj.obj_type, obj.obj_stock,\
                    obj.obj_single, obj.img_id, MIN(pri.pri_credit) AS price FROM t_object_obj obj\
                    Inner Join t_price_pri pri ON pri.obj_id = obj.obj_id\
                    Inner Join tj_obj_poi_jop jop ON jop.obj_id = obj.obj_id\
                    ...
                    AND (not exists(SELECT oli.oli_id FROM tj_object_link_oli oli\
                                WHERE oli.obj_id_child =  obj.obj_id\
                                AND oli.oli_removed =  0)\
                        OR obj.obj_type = 'category')\
